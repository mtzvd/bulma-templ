...
5. **Commit:** Only commit `.templ` files — generated `*_templ.go` files are ignored by git

**Important:** Generated `*_templ.go` files are NOT tracked in git. Users must run `templ generate` after cloning or pulling changes.
...
- `*_templ.go` — Generated files (NOT tracked in git, regenerated by `task templ`)
- `*_test.go` — Unit tests
